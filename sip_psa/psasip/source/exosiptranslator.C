...
RTSipReq convertOsipReq2MCF(osip_message_t *req, RTSipReq mcfReq)
{
	...
	if (NULL != body)
	{
		convertOsipBody2MCF(body, mcfReq.body);
	}

	osip_content_type_t* contentType = NULL;
	contentType = osip_message_get_content_type(req);
	if (contentType != NULL)
	{
		convertOsipContentType2MCF(contentType, mcfReq.content_type);
	}
...
RTSipResp convertOsipResp2MCF(osip_message_t *response, RTSipResp mcfResp)
{
	...
	if (200 == mcfResp.statusCode || 180 == mcfResp.statusCode || 183 == mcfResp.statusCode)
	{
		osip_body_t *body = NULL;
...
{
	/*string callid = osip_call_id_get_number(callId);
	if(strlen(callId->host) != 0)
	{
		callid += string("@") + osip_call_id_get_host(callId);
	}
	printf("call id ttttttttttttttttt %s\n", callid.c_str());
	*/
...
{
	char * ch;
	osip_uri_to_str(osipUri, &ch);
	string uri = ch;
	size_t scheEnd = uri.find(":");
	if(scheEnd != uri.npos)
	{	
		mcfSipUri.scheme = uri.substr(0, scheEnd).c_str();
		size_t userEnd = uri.find("@");
		if(userEnd != uri.npos){
			mcfSipUri.username = uri.substr(scheEnd+1, userEnd-scheEnd-1).c_str();
			string hostname = uri.substr(userEnd+1).c_str();
			size_t hostEnd = hostname.find(":");
			if(hostEnd != hostname.npos){
				mcfSipUri.host = hostname.substr(0, hostEnd).c_str();
				size_t end = hostname.find(";");
				if(end != hostname.npos)
					mcfSipUri.port = hostname.substr(hostEnd+1, end-hostEnd-1).c_str();
				else
					mcfSipUri.port = hostname.substr(hostEnd+1).c_str();
				
			}
			else{
				size_t end = hostname.find(";");
				if(end != hostname.npos)
					mcfSipUri.host = hostname.substr(hostEnd+1, end-hostEnd-1).c_str();
				else
					mcfSipUri.host = hostname.substr(hostEnd+1).c_str();
			}
		}
		else{
			size_t end = uri.find(";");
			if(end != uri.npos)
				mcfSipUri.username = uri.substr(scheEnd+1, end-scheEnd-1).c_str();
			else
				mcfSipUri.username = uri.substr(scheEnd+1).c_str();
...
osip_message_t* convertMCF2OsipInfo(RCTSipInfo mcfMessage,
...
RTSipInfo convertOsipInfo2MCF(osip_message_t * info, RTSipInfo mcfInfo){
	convertOsipUri2MCF(info->req_uri, mcfInfo.req_uri);
	osip_body_t *body = NULL;
	osip_message_get_body(info, 0, &body);
	if (NULL != body)
	{
		convertOsipBody2MCF(body, mcfInfo.body);
	}

	osip_content_type_t* contentType = NULL;
	contentType = osip_message_get_content_type(info);
	if (contentType != NULL)
	{
		convertOsipContentType2MCF(contentType, mcfInfo.content_type);
	}
...
osip_message_t* convertMCF2OsipUpdate(RCTSipUpdate mcfMessage,
		osip_message_t *message) {
	// req_uri
	convertMCF2OsipUri(mcfMessage.req_uri, message->req_uri);
...
RTSipUpdate convertOsipUpdate2MCF(osip_message_t * update, RTSipUpdate mcfUpdate){
	convertOsipUri2MCF(update->req_uri, mcfUpdate.req_uri);
	osip_body_t *body = NULL;
	osip_message_get_body(update, 0, &body);
	if (NULL != body)
	{
		convertOsipBody2MCF(body, mcfUpdate.body);
	}

	osip_content_type_t* contentType = NULL;
	contentType = osip_message_get_content_type(update);
	if (contentType != NULL)
	{
		convertOsipContentType2MCF(contentType, mcfUpdate.content_type);
	}

	return mcfUpdate;
}

osip_message_t* convertMCF2OsipPrack(RCTSipPrack mcfMessage,
		osip_message_t *message) {
	// req_uri
//	convertMCF2OsipUri(mcfMessage.req_uri, message->req_uri);
...
RTSipPrack convertOsipPrack2MCF(osip_message_t * prack, RTSipPrack mcfPrack){
	convertOsipUri2MCF(prack->req_uri, mcfPrack.req_uri);
	osip_body_t *body = NULL;
	osip_message_get_body(prack, 0, &body);
	if (NULL != body)
	{
		convertOsipBody2MCF(body, mcfPrack.body);
	}

	osip_content_type_t* contentType = NULL;
	contentType = osip_message_get_content_type(prack);
	if (contentType != NULL)
	{
		convertOsipContentType2MCF(contentType, mcfPrack.content_type);
	}
