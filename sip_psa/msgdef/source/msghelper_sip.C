...
CVarChar CSipMsgHelper::toString(RCTSipContentType sipContentType)
{
	...
	if (sipContentType.type.length() == 0 || sipContentType.subtype.length() == 0)
	{
		return result;
	}
	sprintf(tmp, "%s/%s", sipContentType.type.c_str(), sipContentType.subtype.c_str());
...
TSipContentType CSipMsgHelper::createSipContentType(const CHAR* type, const CHAR* subtype)
{
	TSipContentType contentType;

	if (type) contentType.type = type;
	if (subtype) contentType.subtype = subtype;
